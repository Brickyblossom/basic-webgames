...
function is_valid(state) {
    for (var row = 1; row <= board.big_cell_size; row++) {
        for (var col = 1; col <= board.big_cell_size; col++) {
            ...
            for (
                var i = (row - 1) * board.big_cell_size + 1;
                i <= row * board.big_cell_size;
                i++
            ) {
                for (
                    var j = (col - 1) * board.big_cell_size + 1;
                    j <= col * board.big_cell_size;
                    j++
                ) {
        ...
        for (var y = 1; y <= board.col; y += board.big_cell_size) {
            for (var i_1 = x; i_1 <= x + board.big_cell_size - 1; i_1++) {
                for (var j_1 = y; j_1 <= y + board.big_cell_size - 1; j_1++) {
                    ...
                    for (
                        var i_2 = x;
                        i_2 <= x + board.big_cell_size - 1;
                        i_2++
                    ) {
                        for (
                            var j_2 = y;
                            j_2 <= y + board.big_cell_size - 1;
                            j_2++
                        ) {
                            if (!reveal[i_2][j_2]) continue;
                            if (i_1 == i_2 && j_1 == j_2) continue;
                            if (reveal[i_1][j_1] == reveal[i_2][j_2]) {
                                let cell_1 = document.getElementById(
                                    hash(i_1, j_1)
                                );
                                let cell_2 = document.getElementById(
                                    hash(i_2, j_2)
                                );
                                if (!start_board[i_1][j_1])
                                    cell_1.style.color = "red";
                                else cell_1.style.backgroundColor = "red";
                                if (!start_board[i_2][j_2])
...
function GenerateBoard(cur_row, cur_col) {
    ...
    for (var i = 0; i < board.num; i++) {
        ...
        if (is_valid(state) == false) {
            ...
            var next_cur_col = cur_col + 1,
                next_cur_row = cur_row;
            if (next_cur_col == board.col + 1)
                next_cur_row++, (next_cur_col = 1);
...
function Reveal_cell() {
    ...
    for (var i = 0; i < hidden_number; i++) {
        ...
        var cell_row = Math.floor((arr[i] - 1) / board.col) + 1,
            cell_col = arr[i] % board.col;
...
function same_big_cell(i, j, x, y) {
    var row_1 = Math.floor((i - 1) / board.big_cell_size) + 1,
        col_1 = Math.floor((j - 1) / board.big_cell_size) + 1;
    var row_2 = Math.floor((x - 1) / board.big_cell_size) + 1,
        col_2 = Math.floor((y - 1) / board.big_cell_size) + 1;
    ...
    for (var i = 1; i <= board.row; i++) {
        for (var j = 1; j <= board.col; j++) {
            if (reveal[i][j] == reveal[row][col]) {
                ...
                if (i == row || j == col || same_big_cell(i, j, row, col))
