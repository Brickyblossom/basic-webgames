...
function init() {
    ...
    document.getElementById(
        "timer"
    ).innerHTML = `<i class="twa twa-timer-clock"></i>: 0`;
    gameBoard.innerHTML = "";
                ...
                for (var x = -1; x <= 1; x++) {
                    for (var y = -1; y <= 1; y++) {
                        ...
                        if (
                            new_i < 1 ||
                            new_j < 1 ||
                            new_i > board_info.cnt_row ||
                            new_j > board_info.cnt_col
                        )
                            continue;
...
function startTimer() {
    board_info.timerID = setInterval(function () {
        ...
        document.getElementById(
            "timer"
        ).innerHTML = `<i class="twa twa-timer-clock"></i>: ${gameTime}`;
    }, 1000);
}
...
function alertbomb() {
    document.getElementById(
        "flags"
    ).innerHTML = `<i class="twa twa-triangular-flag"></i>: ${
        board_info.cnt_bomb - cntflag
    }`;
}
    ...
    if (state[i][j] == count) {
        for (var x = -1; x <= 1; x++) {
            for (var y = -1; y <= 1; y++) {
                ...
                if (rightflag[new_i][new_j] == false)
                    reveal(new_cell, new_i, new_j);
            }
        }
    }
}
...
function cell_click(cell, i, j) {
    ...
    if (board_info.dead == false) alertbomb();
    if (cnt == board_info.cnt_row * board_info.cnt_col - board_info.cnt_bomb)
        toggleEndGame();
}
...
function toggleEndGame() {
    ...
    for (var i = 1; i <= board_info.cnt_row; i++) {
        for (var j = 1; j <= board_info.cnt_col; j++) {
            ...
            let cell = document.getElementById(hash(i, j));
            if (rightflag[i][j] && !flag[hash(i, j)])
